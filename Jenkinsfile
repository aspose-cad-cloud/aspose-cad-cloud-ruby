parameters {
		...
		string(name: 'testServerUrl', defaultValue: 'https://auckland-cad-cloud-staging.dynabic.com', description: 'server url')		
}
...
def runtests(dockerImageVersion)
{
    dir(dockerImageVersion){
        try {
            stage('checkout'){
                checkout([$class: 'GitSCM', branches: [[name: '*/' + params.branch]], doGenerateSubmoduleConfigurations: false, extensions: [[$class: 'WipeWorkspace']], submoduleCfg: [], userRemoteConfigs: [[credentialsId: '9d6c4dfa-042c-4ed1-81c7-9175179dddda', url: 'https://github.com/aspose-cad-cloud/aspose-cad-cloud-ruby.git/']]])
